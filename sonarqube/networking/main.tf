...
resource "aws_route_table" "pubRT" {
  ...
  route {
    ...
    vpc_peering_connection_id = aws_vpc_peering_connection.sonar-vpc-peering.id
  }

  tags = {
    Name = var.public_RT_name
  }
  depends_on = [aws_vpc_peering_connection.sonar-vpc-peering]
}
...
resource "aws_vpc_peering_connection" "sonar-vpc-peering" {
  peer_vpc_id = aws_vpc.sonar.id ##our vpc id
...
resource "aws_route" "default_vpc_to_peer" {
  ...
  destination_cidr_block    = aws_vpc.sonar.cidr_block
  vpc_peering_connection_id = aws_vpc_peering_connection.sonar-vpc-peering.id
  depends_on                = [aws_vpc_peering_connection.sonar-vpc-peering]
}
